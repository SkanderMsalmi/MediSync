...
export default function Appointment() {
  ...
  const getFilteredByDoctors = () => {
    axios
    .get('http://localhost:8081/hospital/show/adam/getFilteredAppointmentByDoctor')
    .then((response) => {
      // Update the state with the fetched data
      setAppointments(response.data);
    })
  ...
  const getFilteredByNurses = () => {
    axios
    .get('http://localhost:8081/hospital/show/adam/getFilteredAppointmentByNurse')
    .then((response) => {
      // Update the state with the fetched data
      setAppointments(response.data);
    })
  ...
  const getFilteredByMonth = () => {
    axios
    .get('http://localhost:8081/hospital/show/adam/getFilteredAppointmentByMonth')
    .then((response) => {
      // Update the state with the fetched data
      setAppointments(response.data);
    })
